...
function migrate(v){
 ...
 v.employees.forEach(e=>{e.occupation ||= 'その他'});
...
function renderBoard(board){
 ...
 list.forEach(e=>{const n=t.content.cloneNode(true),card=n.querySelector('.employee-card');const bg=data.settings.statusColors[e.status]||DEFAULT_COLORS[e.status]||'#fff';card.style.setProperty('--status-bg',bg);card.style.setProperty('--card-text',contrast(bg));n.querySelector('.employee-name').textContent=e.name;n.querySelector('.employee-meta').textContent=[e.department,e.occupation,e.role].filter(Boolean).join(' / ');n.querySelector('.status-pill').textContent=e.status;n.querySelector('.destination').textContent=e.destination||'―';n.querySelector('.purpose').textContent=e.purpose||' ';n.querySelector('.return-time').textContent=e.returnTime||'―';n.querySelector('.phone-status').textContent=phoneText(e.phone);const tags=n.querySelector('.tag-row');if(e.id===data.settings.currentUserId)tags.innerHTML+='<span class="tag">自分</span>';if(e.direct)tags.innerHTML+='<span class="tag">直行</span>';if(e.goHome)tags.innerHTML+='<span class="tag">直帰</span>';if(e.memo)tags.innerHTML+=`<span class="tag">${esc(e.memo)}</span>`;n.querySelector('.change-btn').addEventListener('click',()=>openEdit(e.id));board.appendChild(n)});
